def repoName = "hippy96/samplespring"  //The repo the app will be pushed to
        ...
        stage('app deploy') {
            ...
            steps {
                withKubeConfig([credentialsId: 'creds-kubernetes', serverUrl: "${kubeMaster}"]) {
                    ...
                    sh 'kubectl rollout restart deployment/sample-spring-deployment -n test'
                    sh 'kubectl rollout restart deployment/sample-spring-deployment -n prod'  //The deployment name should be parameterized
                }
            }
            post {
                failure {
